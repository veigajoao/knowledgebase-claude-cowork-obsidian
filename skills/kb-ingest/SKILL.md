...
material, extract what matters, and write a structured ingest report — without
touching the raw files themselves.

## Core constraint

`raw/` is read-only ground truth. Never edit, rename, move, or delete anything in
it. All your output goes to `index/`.

## What "unprocessed" means

A file in `raw/` is unprocessed if it doesn't appear in `index/ingest-log.md`. If
that file doesn't exist yet, treat all files in `raw/` as unprocessed.
...
### 2. Scan raw/ for new files
List all files in `raw/` (recursively). Identify those not yet in the ingest log.
Common file types you'll encounter: `.md`, `.txt`, `.pdf`, `.html`, `.png`, `.jpg`.

### 3. For each unprocessed file, extract:
...
articles you're recommending for creation or enrichment.
